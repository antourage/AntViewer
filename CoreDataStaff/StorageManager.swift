...
extension StorageManager {
  ...
  private func loadVideoContent(content: VideoContent) -> VideoContentMO {
...
public extension StorageManager {
  ...
  func loadChat(for videoContent: VideoContent) -> [Message]? {
    let content = loadVideoContent(content: videoContent)
    guard let messages = content.messages?.allObjects as? [MessageMO] else {
      print("Error fetching chat")
      return nil
    }
    return messages.map { Message(userID: $0.userId, timestamp: Int($0.timestamp), nickname: $0.nickname, text: $0.text) }.sorted { $0.timestamp < $1.timestamp }
  }

  func saveChat(for videoContent: VideoContent, value: [Message]) {
    defer { saveContext() }
    let contentToSave = loadVideoContent(content: videoContent)
    contentToSave.addToMessages(NSSet(array: value))
  }


  func loadPolls(for videoContent: VideoContent) -> [Poll]? {
    //TODO: fetch polls for vod/live
    return nil
  }
